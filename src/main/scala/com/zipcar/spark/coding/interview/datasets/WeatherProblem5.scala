...
object WeatherProblem5 {
  
  def main(args: Array[String]) = {
    
    if(args.length < 2) {
      println("Usage:\n\n\t bin/spark-submit.cmd --class " + WeatherProblem5.getClass.getName + " <jar_file_name> <inputFilePath> <outputFilePath>")
      System.exit(1)
    }
    ...
    val weather_station_names_distinct = weather_station_names_upper.distinct().orderBy("value")
